...
async def get_user_posts_by_username(username: str, current_user: dict = Depends(get_current_user)):
    try:
        ...
        post_service_response = httpx.get(
            f"{POST_SERVICE_URL}/posts/user/{username}",
            headers={"Authorization": f"Bearer {current_user['token']}"}
        )
...
async def create_post(post_data: PostCreate, current_user: dict = Depends(get_current_user)):
    try:
        # Hash the user_id before sending it to the Post Service
        post_data.user_id = bcrypt.hashpw(current_user['token'].encode(), bcrypt.gensalt())
        post_data.timestamp = str(datetime.utcnow())  # Set the timestamp to current UTC time
...
@post_router.post("/post/{post_id}/like", response_model=PostResponse, tags=["post"])
async def update_post(post_id: str, current_user: dict = Depends(get_current_user)):
    try:
        # Hash the user_id before sending it to the Post Service
        user_id = bcrypt.hashpw(current_user['token'].encode(), bcrypt.gensalt())
        post_service_response = httpx.post(
            f"{POST_SERVICE_URL}/posts/{post_id}/like",
...
async def update_post(post_id: str, current_user: dict = Depends(get_current_user)):
    try:
       ...
       # user_id_base64 = (user_id).decode('utf-8')  # Convert to base64
        post_service_response = httpx.post(
            f"{POST_SERVICE_URL}/posts/{post_id}/like",
            json={'user_id': str(user_id)},  # Include the base64-encoded user_id in the JSON payload
            headers={"Authorization": f"Bearer {current_user['token']}"}
        )
        return post_service_response.json()
    except Exception as e:
